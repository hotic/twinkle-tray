...
const BrightnessPanel = memo(function BrightnessPanel() {
  ...
  const handleThemeUpdated = () => setThemeVersion(prev => prev + 1)
  ...
  const getThemeToggleIcon = () => {
    ...
    return (
      <div title={T.t("PANEL_BUTTON_TOGGLE_THEME")} className="theme-toggle" onClick={handleThemeToggle}>
        {isDark ? "\uE706" : "\uE708"}
      </div>
    )
  }
  ...
  const getMonitors = () => {
    if (!state.monitors || numMonitors == 0) {
      ...
      if (window.settings?.sdrAsMainSlider) {
        const hasActiveHDR = Object.values(state.monitors).some(monitor => monitor?.hdr === "active")
        if (!hasActiveHDR) {
          return (
      ...
      if (state.linkedLevelsActive) {
        ...
        for (const key in state.monitors) {
          ...
          if (supportsPrimarySlider(monitor)) {
        ...
        return sorted.map((monitor) => {
          ...
          const showHDRSliders = (monitor.hdr === "active" || window.settings?.hdrDisplays?.[monitor.key]) && !window.settings?.sdrAsMainSlider
          const canRenderPrimarySlider = supportsPrimarySlider(monitor)

          if (!canRenderPrimarySlider && !isHDROnlySDR) {
            return (<div key={monitor.key}></div>)
          }
          ...
          if (!useFeatures || !hasFeatures) {
            ...
            return (
              <div className="monitor-sliders extended" key={monitor.key}>
                ...
                { !isHDROnlySDR && canRenderPrimarySlider && (
